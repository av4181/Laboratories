create or replace PACKAGE BODY PKG_SAMENS1_M7_LABORATORIES
AS
...
--         p_laboratory_name IN Laboratories.Laboratory_Name%TYPE,
    ...
    AS
--         v_laboratory LABORATORY.LABORATORY_ID%TYPE:= lookup_laboratory(p_laboratory_name);
...
--                 FETCH FIRST 1 ROW ONLY;
                ...
                -- debugging
               -- DBMS_OUTPUT.PUT_LINE('Technician ' || i || ': Laboratory_ID = ' || t_technicians_new(i).Laboratory_ID);

                --                 IF NOT EXISTS (SELECT 1 FROM Laboratories WHERE Laboratory_ID = t_technicians_new(i).Laboratory_ID) THEN
--                     DBMS_OUTPUT.PUT_LINE('ERROR: Invalid Laboratory_ID for technician ' || i);
--                     RAISE_APPLICATION_ERROR(-20001, 'Invalid Laboratory_ID selected');
--                 END IF;
--                 -- debugging
                ...
                -- Start einddatum tot max 5 jaar geleden
                t_technicians_new(i).Function_Start_Date := random_date(SYSDATE - 1825, SYSDATE);
                t_technicians_new(i).Function_End_Date   :=
                        CASE
                            WHEN random_number(1, 10) <= 8 -- om vast te leggen hoeveel % een einddatum NULL hebben en dus de firma niet hebben verlaten
                                THEN NULL
        ...
        DBMS_OUTPUT.PUT_LINE(TO_CHAR(SYSTIMESTAMP, '[YYYY-MM-DD HH24:MI:SS]  ') ||
                             ...
                             timestamp_diff(SYSTIMESTAMP,v_starttimestamp) || ' seconds.' || u'\000D\000A' ||
                             'original amount: ' || t_original_amount_technicians.COUNT || ' Current amout: ' ||
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE(TO_CHAR(SYSTIMESTAMP, '[YYYY-MM-DD HH24:MI:SS]  ') ||
                             ...
                             (p_amount) || ' lines. The generation time for this procedure was ' ||
                             timestamp_diff(SYSTIMESTAMP, v_starttimestamp) || ' seconds.' || u'\000D\000A' ||
                             'original amount: ' || t_original_amount_instruments.COUNT || ' Current amout: ' ||
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE(TO_CHAR(SYSTIMESTAMP, '[YYYY-MM-DD HH24:MI:SS]  ') ||
                             ...
                             (p_count) || ' lines. The generation time for this procedure was ' ||
                             timestamp_diff(SYSTIMESTAMP, v_starttimestamp) || ' seconds.' || u'\000D\000A' ||
                             'original amount: ' || t_original_amount_samples.COUNT || ' Current amout: ' ||
                             (t_original_amount_samples.COUNT + (p_count)) || '.');
